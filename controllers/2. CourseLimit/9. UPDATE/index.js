...
const COURSE_UPDATE_LIMIT_INVOICE = async (req , res) => {
    try 
    {
        ...
        const existData = await prismaService.course_limit_invoice.findUnique({
            where: {
                id: parseInt(id),
        ...
        try 
        {
            ...
            if(checkResponse?.data?.data?.status === 'paid')
            {
                ...
                const result = await prismaService.course_limit_invoice.update({
                    where: {
                        id:parseInt(id)
                    },
                    data: {
...
module.exports = COURSE_UPDATE_LIMIT_INVOICE