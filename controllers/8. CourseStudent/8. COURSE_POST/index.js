...
// Автоматаар код үүсгэх функц
const generateStudentCode = async (courseId) => {
    try {
        // Хамгийн сүүлд бүртгэгдсэн суралцагчийн кодыг авах
        ...
        // Үндсэн префикс формат: DL[XX]S[YYY]
        const basePrefix = "DL";
        const suffix = "S";
        
        if (!lastStudent || !lastStudent.kode) {
            // Хэрэв өмнөх суралцагч байхгүй бол эхний код
            return `${basePrefix}01${suffix}001`;
        }

        const lastCode = lastStudent.kode;
        
        // Код формат шалгах (DL + 2 оронтой дугаар + S + 3 оронтой дугаар)
        // Жишээ: DL01S001, DL02S999
        ...
        // Хэрэв формат буруу бол эхний код буцаах
        return `${basePrefix}01${suffix}001`;
...
const COURSE_POST_STUDENT = async (req, res) => {
    try 
    {
        ...
        const { familyname, firstname, lastname, register, gender, bloodtype, city, district, location, phone, birthdate, category } = req.body;
